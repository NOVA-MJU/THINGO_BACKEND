...
| order | 정렬 키 |
|-------|---------|
| relevance | `ts_rank*0.6 + 제목매칭 부스트(하나라도 0.25 / 전부 0.40) + popularity*0.0001 + 카테고리·타입 가중치 + 인기검색어 보너스 + 유효성 가중치` |
| latest | `date DESC` |
| oldest | `date ASC` |
...
> 유효성 보정은 relevance 정렬에만 적용하며 응답 스키마는 불변(`valid_until` 미노출).
