...
점수 = (검색어와 본문/제목이 얼마나 맞나)        … 기본 관련도(ts_rank)
     ...
     + 도메인/카테고리 가중치 (뉴스·방송은 하향)
     + 실시간 인기검색어와 겹치면 보너스
     + 유효성(기한) 반영                          … 아래 "유효성" 설명
     + 최신성(최근 글일수록 ↑, 공고는 더 강하게)   … 아래 "최신성" 설명
```
...
학교 정보는 **최신이 특히 중요**합니다. 그래서 같은 키워드에서 관련도가 비슷하면 **더 최근에
올라온 글이 위로** 오도록 발행일을 점수에 반영합니다.

- **공고(공지·학과공지·총학공지)** 는 최신성이 특히 중요해 더 강하게 우대합니다.
- 시간이 지날수록 우대 폭이 줄어듭니다(대략 한 달이면 절반, 두 달이면 1/4, 수개월이면 거의 0).
- **뉴스·방송**은 학교 공식 정보(공지·일정)보다 우선순위를 낮췄습니다. 같은 키워드면 공지가 위,
  뉴스가 아래로 갑니다.

> 이 역시 `relevance` 정렬에만 적용됩니다. `latest`/`oldest`는 순수 날짜순이라 영향받지 않습니다.
...
- **같은 공지가 여러 카테고리에 올라오면 중복으로 나오나요?** 아니요. 같은 원문(link)은 **1건으로
  합쳐** 보여줍니다. 실시간 반영·전체 동기화 모두 link 기준으로 중복을 정리합니다.
